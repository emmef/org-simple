#ifndef ORG_SIMPLE_SAMPLERATE_H
#define ORG_SIMPLE_SAMPLERATE_H
/*
 * org-simple/frequency.h
 *
...
public:
  ...
  static constexpr bool too_high(double value) { return v > 1e+100; }
  ...
  SampleRate(double rate) : rate_(validated(rate)) {}
  ...
  template <typename R> bool operator<=(R other) const {
    return rate_ M = (double)other;
  }

  template <typename R> SampleRate &operator<<(R newRate) {
    rate_ = validated(newRate);
    return *this;
  }

  template <typename R> SampleRate &operator>>(R &target) const {
